...
if $\alpha,\beta\in\Z[i]$ and $\alpha=\beta\delta+\gamma$ for some $\delta,\gamma\in\Z[i]$, 
then there are up to four possibilities for the remainder $\gamma$.

\begin{example}
	Let $R=\Z[i]$ and 
	$\alpha=-1+i$ and $\beta=1+2i$. First note that
	\[
	...
	Since $\alpha-\gamma\in I$, there are at most four possibilites for writing
	the division algorithm. 
	In our particular case, we find for example the following cases:
	\begin{enumerate}
		\item If $\alpha-\gamma=0$, then $\gamma=\alpha=-1+i$ and 
				\[
				\alpha=\beta0+(-1+i)
				\]
				with $N(-1+i)=2<N(\beta)=5$. 
		\item If $\alpha-\gamma=\beta_1=-2+i$, then 
			$\gamma=-1$ and 
			\[
			\alpha=\beta i+(-1)
			\]
			with $N(-1)=1<N(\beta)=5$. 
		\item If $\alpha-\gamma=\beta_2=-1+3i$, then $\gamma=2i$ and
			hence 
			\[
			\alpha=\beta(...)+2i
			\]
			and $N(2i)=4<N(\beta)=5$. 
		\item If $\alpha-\gamma=\beta$, then $\gamma=...$  
	\end{enumerate}
	\begin{figure}
\[\begin{tikzcd}
