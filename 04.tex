...
\begin{exercise}
\label{xca:Gauss_primitive}
A polynomial $f\in\Z[X]$ is \textbf{primitive} if
the greatest common divisors of its coefficients is equal to one. Prove the following statements:
\begin{enumerate}
    \item Let $f,g\in\Z[X]$ be primitive polynomials. Prove that $fg$ es primitive.
    \item Let $f\in\Z[X]$ be non-constant and primitive. Then
$f$ is irreducible in $\Z[X]$ if and only if 
$f$ is irreducible in $\Q[X]$. 
\end{enumerate}
\end{exercise}

The first item of Exercise \ref{xca:Gauss_primitive} is known as Gauss' lemma.
