...
\begin{definition}
    ...
    a representation. A subspace $W$ of $V$ is said to be \emph{invariant} (with respect to $\rho$)
...
\begin{example}
 ...
 is an invariant map, then the \emph{kernel} 
	...
	\ker T=\{v\in V:T(v)=0\}
	\]
	is an invariant of $V$ and the \emph{image} 
	\[
		T(V)=\{T(v):v\in V\}
	\]
...
\index{Subrepresentation}
    Let $\rho\colon G\to\GL(V)$
    a representation. A \emph{subrepresentation} of $\rho$ is a restricted representation 
