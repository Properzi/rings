...
\end{cases}
\]
If $\rho\colon G\to\GL(V)\simeq\GL_n(\C)$ is a representation of a finite group $G$, then
...
\begin{proof}
     ...
     of vectors from $L(G)$ have at most $|G|$ elements. By
...
If $g\in G$, the conjugacy class of $g$ in $G$ 
...
\begin{theorem}
	...
	$\rho=\sum_{j=1}^rm_j\rho_j$ where $m_1,\dots m_r\in\Z_{\geq0}$, then
...
\begin{corollary}
    A representation $\rho$ of a finite group is irreducible if and only if $\langle\chi_\rho,\chi_\rho\rangle=1$.
\end{corollary}
